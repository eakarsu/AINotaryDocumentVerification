...
// Custom Feature: Video attestation
// Video attestation: Record notary verification session (with consent); store video proof alongside document scans for liability protection
import React, { useState } from 'react';

export default function CFVideoAttestationPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-video-attestation/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Video attestation</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Video attestation: Record notary verification session (with consent); store video proof alongside document scans for liability protection</p>
