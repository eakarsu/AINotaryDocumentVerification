...
const formFields = [
  ...
  { key: 'method', label: 'Payment Method', type: 'select', options: ['Credit Card', 'Debit Card', 'Bank Transfer', 'Check', 'Cash', 'PayPal', 'Stripe'] },
  { key: 'status', label: 'Status', type: 'select', options: ['pending', 'completed', 'failed', 'refunded', 'cancelled'], defaultValue: 'pending' },
  ...
  return (
    <div className="feature-page">
      <div className="page-header">
        <h1>{'\u{1F4B3}'} Payments</h1>
        <button className="btn btn-primary" onClick={handleCreate}>+ Add New</button>
      </div>
      <DataTable columns={columns} data={data} onRowClick={handleRowClick} />
      {showDetail && selected && <DetailModal title={`Payment: ${selected.client_name || selected.id}`} data={selected} onClose={() => setShowDetail(false)} onEdit={handleEdit} onDelete={handleDeleteClick} />}
      {showForm && <FormModal title={editing ? 'Edit Payment' : 'New Payment'} fields={formFields} initialData={editing} onSubmit={handleSubmit} onClose={() => setShowForm(false)} />}
      {showDelete && <DeleteConfirm message="Delete this payment record?" onConfirm={handleDelete} onCancel={() => setShowDelete(false)} />}
    </div>
  )
}
