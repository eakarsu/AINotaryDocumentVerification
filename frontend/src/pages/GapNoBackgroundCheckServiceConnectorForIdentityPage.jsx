...
// Gap (Non-AI): No background check service connector (for identity verification)
// No background check service connector (for identity verification)
import React, { useState } from 'react';

export default function GapNoBackgroundCheckServiceConnectorForIdentityPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-background-check-service-connector-for-identity/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No background check service connector (for identity verification)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No background check service connector (for identity verification)</p>
