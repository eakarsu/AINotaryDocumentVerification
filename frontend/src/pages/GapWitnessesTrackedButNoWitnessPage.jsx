...
// Gap (AI): Witnesses tracked but no '/witness
// Witnesses tracked but no /witness-vetting AI (flag suspicious witness patterns)
import React, { useState } from 'react';

export default function GapWitnessesTrackedButNoWitnessPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-witnesses-tracked-but-no-witness/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Witnesses tracked but no '/witness</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Witnesses tracked but no /witness-vetting AI (flag suspicious witness patterns)</p>
