...
// Gap (Non-AI): No webhooks for external triggers (e.g., new client created in CRM)
// No webhooks for external triggers (e.g., new client created in CRM)
import React, { useState } from 'react';

export default function GapNoWebhooksForExternalTriggersEGNewClientCPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-external-triggers-e-g-new-client-c/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for external triggers (e.g., new client created in CRM)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for external triggers (e.g., new client created in CRM)</p>
