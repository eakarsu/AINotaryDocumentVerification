...
// Gap (AI): Payments recorded but no '/fee
// Payments recorded but no /fee-optimization (suggest standard or premium tiers)
import React, { useState } from 'react';

export default function GapPaymentsRecordedButNoFeePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-payments-recorded-but-no-fee/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Payments recorded but no '/fee</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Payments recorded but no /fee-optimization (suggest standard or premium tiers)</p>
