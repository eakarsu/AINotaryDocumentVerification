...
// Gap Feature: No webhooks for external triggers (e.g., new client created in CRM)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / gap-no-webhooks-for-external-triggers-e-g-new-client-c',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No webhooks for external triggers (e.g., new client created in CRM)'.\nProject: AINotaryDocumentVerification.\nContext: No webhooks for external triggers (e.g., new client created in CRM)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-webhooks-for-external-triggers-e-g-new-client-c', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-webhooks-for-external-triggers-e-g-new-client-c', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-webhooks-for-external-triggers-e-g-new-client-c']
    );
