...
// Gap Feature: Clients exist without \'/client
// Clients exist without /client-risk-score (detect problematic repeat customers)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / gap-clients-exist-without-client',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Clients exist without \'/client'.\nProject: AINotaryDocumentVerification.\nContext: Clients exist without /client-risk-score (detect problematic repeat customers)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['clients-exist-without-client', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'clients-exist-without-client', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
