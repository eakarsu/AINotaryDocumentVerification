...
// When multiple documents involve same parties, auto-link them and surface patterns (e.g., repeated amendments to same agreement)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / cf-multi-document-correlation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_multi_document_correlation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-multi-document-correlation feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Multi-document correlation'.
Context: When multiple documents involve same parties, auto-link them and surface patterns (e.g., repeated amendments to same agreement)
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-multi-document-correlation/run error:', err);
