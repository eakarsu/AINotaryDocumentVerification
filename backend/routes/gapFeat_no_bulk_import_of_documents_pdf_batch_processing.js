...
// Gap Feature: No bulk import of documents (PDF batch processing)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / gap-no-bulk-import-of-documents-pdf-batch-processing',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No bulk import of documents (PDF batch processing)'.\nProject: AINotaryDocumentVerification.\nContext: No bulk import of documents (PDF batch processing)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-bulk-import-of-documents-pdf-batch-processing', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-bulk-import-of-documents-pdf-batch-processing', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
