...
// Record notary verification session (with consent); store video proof alongside document scans for liability protection
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / cf-video-attestation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_video_attestation`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-video-attestation feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Video attestation'.
Context: Record notary verification session (with consent); store video proof alongside document scans for liability protection
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-video-attestation/run error:', err);
