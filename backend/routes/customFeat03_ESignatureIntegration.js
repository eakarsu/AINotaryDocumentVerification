...
// Embed DocuSign/HelloSign; auto-route documents to witness e-signature, reducing in-person visits
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / cf-e-signature-integration',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_e_signature_integration`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-e-signature-integration feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'E-signature integration'.
Context: Embed DocuSign/HelloSign; auto-route documents to witness e-signature, reducing in-person visits
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-e-signature-integration/run error:', err);
