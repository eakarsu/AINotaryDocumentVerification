...
// Allow notaries to accept remote jobs; build geographic dispatch engine for urgent same-day notarizations
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / cf-notary-marketplace',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_notary_marketplace`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-notary-marketplace feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Notary marketplace'.
Context: Allow notaries to accept remote jobs; build geographic dispatch engine for urgent same-day notarizations
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-notary-marketplace/run error:', err);
