...
// Autonomous background process that reviews new notarizations against updated state regulations, flags issues, and emails notary proactively
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / cf-agentic-compliance-monitoring',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_compliance_monitoring`;
(async () => {
  try {
    await pool.query(`
...
// POST /run — execute the cf-agentic-compliance-monitoring feature
router.post('/run', auth, aiRateLimiter, async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic compliance monitoring'.
Context: Autonomous background process that reviews new notarizations against updated state regulations, flags issues, and emails notary proactively
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-compliance-monitoring/run error:', err);
