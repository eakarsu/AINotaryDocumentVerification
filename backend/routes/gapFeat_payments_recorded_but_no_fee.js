...
// Gap Feature: Payments recorded but no \'/fee
// Payments recorded but no /fee-optimization (suggest standard or premium tiers)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / gap-payments-recorded-but-no-fee',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Payments recorded but no \'/fee'.\nProject: AINotaryDocumentVerification.\nContext: Payments recorded but no /fee-optimization (suggest standard or premium tiers)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['payments-recorded-but-no-fee', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'payments-recorded-but-no-fee', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
