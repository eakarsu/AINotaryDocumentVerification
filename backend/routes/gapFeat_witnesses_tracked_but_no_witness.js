...
// Gap Feature: Witnesses tracked but no \'/witness
// Witnesses tracked but no /witness-vetting AI (flag suspicious witness patterns)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AINotaryDocumentVerification / gap-witnesses-tracked-but-no-witness',
    },
    body: JSON.stringify({
...
router.post('/run', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Witnesses tracked but no \'/witness'.\nProject: AINotaryDocumentVerification.\nContext: Witnesses tracked but no /witness-vetting AI (flag suspicious witness patterns)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['witnesses-tracked-but-no-witness', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'witnesses-tracked-but-no-witness', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['witnesses-tracked-but-no-witness']
    );
